...
test_that("impact_calculation, method1", {
  ...
  expect_equal(a$impact[a$country %in% pine], b$value[b$country %in% pine])
...
test_that("impact_calculation, method2", {
  ...
  b <- b[order(b$index, b$country, b$year, b$age), ]
  expect_equal(a$impact[a$country %in% pine], b$impact[b$country %in% pine])
...
test_that("impact_calculation, method2-HepB-specific", {
  ...
  b <- impact[impact$cohort %in% cohort_min:cohort_max, ]
  b <- b[order(b$index, b$country, b$year, b$age), ]
  expect_equal(a$impact, b$impact[b$country %in% a$country])
